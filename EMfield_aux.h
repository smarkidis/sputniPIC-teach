...
inline void field_aux_allocate(struct grid* grd, struct EMfield_aux* field_aux){
    // Electrostatic potential
    field_aux->Phi = newArr3(FPfield, grd->nxc, grd->nyc, grd->nzc);
    
    // allocate 3D arrays
    field_aux->Exth  = newArr3(FPfield, grd->nxn, grd->nyn, grd->nzn);
    field_aux->Eyth  = newArr3(FPfield, grd->nxn, grd->nyn, grd->nzn);
    field_aux->Ezth  = newArr3(FPfield, grd->nxn, grd->nyn, grd->nzn);
    // B on centers
    field_aux->Bxc = newArr3(FPfield, grd->nxc, grd->nyc, grd->nzc);
    field_aux->Byc = newArr3(FPfield, grd->nxc, grd->nyc, grd->nzc);
    field_aux->Bzc = newArr3(FPfield, grd->nxc, grd->nyc, grd->nzc);
    
}

/** deallocate */
inline void field_aux_deallocate(struct grid* grd, struct EMfield_aux* field_aux){
    // Eth
